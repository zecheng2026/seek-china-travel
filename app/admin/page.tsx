...
export default function AdminPage() {
  ...
  return <main className="adminShell">
    ...
    <section className="adminWorkspace">
      ...
      {active === "dashboard" ? <控制台 onNavigate={setActive} /> : active === "media" ? <MediaLibrary /> : active === "homepage_sections" ? <HeroManager rows={rows} onRefresh={() => loadTable("homepage_sections")} /> : active === "destinations" ? <DestinationManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("destinations")} /> : active === "tours" ? <TourManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("tours")} /> : active === "site_settings" ? <SiteChromeManager /> : active === "travel_guides" ? <GuideManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("travel_guides")} /> : active === "inquiries" ? <InquiryManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("inquiries")} /> : <Collection table={active} title={title} rows={rows} loading={loading} message={message} onRefresh={() => loadTable(active)} />}
    </section>
  </main>;
}

function SiteChromeManager(){const supabase=useMemo(()=>createClient(),[]);const base={header:"Home|/\nTours|/tours\nDestinations|/destinations\nTravel Guide|/travel-guide\nAbout Us|/about\nContact Us|#contact",footerExplore:"Tours|/tours\nDestinations|/destinations\nTravel Guide|/travel-guide\nAbout Us|/about",footerPlan:"Tailor-made Trips|/quote\nSmall Groups|/quote\nGet a Quote|/quote",footerConnect:"Email Us|mailto:hello@seekchinatravel.com\nContact Us|/quote",ctaLabel:"Get a Quote",ctaHref:"/quote",footerText:"Meaningful journeys, thoughtfully designed by people who call China home.",footerSlogan:"Discover China. Your Way."};const [d,setD]=useState(base);const [msg,setMsg]=useState("");const [saving,setSaving]=useState(false);useEffect(()=>{supabase.from("homepage_sections").select("*").eq("section_key","site:chrome").maybeSingle().then(({data})=>{if(!data?.content)return;try{const x=typeof data.content==="string"?JSON.parse(data.content):data.content;const lines=(a:{label:string;href:string}[])=>(a||[]).map(v=>v.label+"|"+v.href).join("\n");setD({...base,header:lines(x.header),footerExplore:lines(x.footerExplore),footerPlan:lines(x.footerPlan),footerConnect:lines(x.footerConnect),ctaLabel:x.headerCta?.label||base.ctaLabel,ctaHref:x.headerCta?.href||base.ctaHref,footerText:x.footerText||base.footerText,footerSlogan:x.footerSlogan||base.footerSlogan});}catch{}})},[supabase]);const parse=(v:string)=>v.split("\n").map(x=>x.trim()).filter(Boolean).map(x=>{const i=x.indexOf("|");return{label:(i<0?x:x.slice(0,i)).trim(),href:(i<0?"#":x.slice(i+1)).trim()}});async function save(){setSaving(true);setMsg("");const content={header:parse(d.header),footerExplore:parse(d.footerExplore),footerPlan:parse(d.footerPlan),footerConnect:parse(d.footerConnect),headerCta:{label:d.ctaLabel,href:d.ctaHref},footerText:d.footerText,footerSlogan:d.footerSlogan};const {data}=await supabase.from("homepage_sections").select("id").eq("section_key","site:chrome").maybeSingle();const payload={section_key:"site:chrome",section_name:"顶部导航与页脚",title:"Site Navigation",content:JSON.stringify(content),is_active:true};const {error}=data?.id?await supabase.from("homepage_sections").update(payload).eq("id",data.id):await supabase.from("homepage_sections").insert(payload);setMsg(error?"保存失败："+error.message:"已保存，前台顶部和底部链接已同步。");setSaving(false)}const area=(label:string,key:"header"|"footerExplore"|"footerPlan"|"footerConnect")=><label style={{gridColumn:"1/-1"}}><span>{label}</span><textarea rows={5} value={d[key]} onChange={e=>setD({...d,[key]:e.target.value})}/><small>每行一个：显示文字|链接地址</small></label>;return <section className="adminPanel"><header><div><h2>顶部导航与页脚</h2><p>直接修改官网顶部菜单、底部文字和链接。前台仍保持英文。</p></div></header><div className="destinationCard"><div className="destinationFormGrid">{area("顶部导航菜单","header")}<label><span>顶部按钮文字</span><input value={d.ctaLabel} onChange={e=>setD({...d,ctaLabel:e.target.value})}/></label><label><span>顶部按钮链接</span><input value={d.ctaHref} onChange={e=>setD({...d,ctaHref:e.target.value})}/></label>{area("底部 Explore 链接","footerExplore")}{area("底部 Plan 链接","footerPlan")}{area("底部 Connect 链接","footerConnect")}<label style={{gridColumn:"1/-1"}}><span>底部公司简介</span><textarea rows={3} value={d.footerText} onChange={e=>setD({...d,footerText:e.target.value})}/></label><label style={{gridColumn:"1/-1"}}><span>底部品牌标语</span><input value={d.footerSlogan} onChange={e=>setD({...d,footerSlogan:e.target.value})}/></label></div>{msg&&<p className="tourSuccess">{msg}</p>}<button className="adminPrimary" disabled={saving} onClick={save}>{saving?"正在保存…":"保存顶部与页脚"}</button></div></section>}
