...
export default function Home() {
  const supabase=useMemo(()=>createClient(),[]); const [liveTours,setLiveTours]=useState<Record<string,unknown>[]>([]); const [liveDestinations,setLiveDestinations]=useState<Record<string,unknown>[]>([]); const [liveGuides,setLiveGuides]=useState<Record<string,unknown>[]>([]); const defaultServices=[{title:"China Travel Concierge",text:"Personal support for planning your private China journey.",image:images.guilin,icon:"◎"},{title:"Fully Managed Trips",text:"Multi-city journeys managed from planning to daily coordination.",image:images.beijing,icon:"▦"},{title:"Itinerary Planning",text:"Smart routes, daily schedules and a comfortable travel pace.",image:images.xian,icon:"⌖"},{title:"Driver & Guide",text:"Private drivers and professional English-speaking local guides.",image:images.chongqing,icon:"◆"},{title:"Private China Tours",text:"Well-designed experiences for couples, families and small groups.",image:images.zhangjiajie,icon:"●"}]; const [services,setServices]=useState(defaultServices);
  useEffect(()=>{Promise.all([supabase.from("tours").select("*").eq("is_published",true).order("sort_order",{ascending:true}).limit(3),supabase.from("destinations").select("*").eq("is_active",true).order("sort_order",{ascending:true}).limit(6),supabase.from("travel_guides").select("*").order("created_at",{ascending:false}).limit(3),supabase.from("homepage_sections").select("content").eq("section_key","home:services").maybeSingle()]).then(([t,d,g,s])=>{setLiveTours((t.data as Record<string,unknown>[]|null)??[]);setLiveDestinations((d.data as Record<string,unknown>[]|null)??[]);setLiveGuides(((g.data as Record<string,unknown>[]|null)??[]).filter(x=>x.is_published!==false&&x.is_active!==false));if(s.data?.content)try{const x=typeof s.data.content==="string"?JSON.parse(s.data.content):s.data.content;if(Array.isArray(x?.items)&&x.items.length)setServices(x.items)}catch{}});},[supabase]);
  return <main>
    ...
    <section className="homeStrengthBar homeServiceBar visualServiceBar" aria-label="SEEK CHINA TRAVEL professional services">
      {services.map((item:any,index:number)=><article key={index}><div className="serviceVisual" style={{backgroundImage:item.image?"url('"+item.image+"')":undefined}}><i>{item.icon||"✦"}</i></div><div><strong>{item.title}</strong><span>{item.text}</span></div></article>)}
    </section>
