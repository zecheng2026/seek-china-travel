...
type TableName = Exclude<(typeof sections)[number][1], "dashboard" | "media" | "about_us" | "contact_us" | "home_services">;
type RecordRow = Record<string, unknown>;

const editableTables = new Set(["site_settings", "homepage_sections", "destinations", "tours", "travel_guides"]);
const heroPages=[["home","首页"],["tours","Tours 页面"],["destinations","Destinations 页面"],["travel-guide","Travel Guide 页面"],["about","About Us 页面"],["contact","Contact Us 页面"],["quote","Plan Your Trip 页面"]] as const;
function navIcon(key: string) {
  const icons: Record<string, string> = {
    dashboard: "⌂", site_settings: "⚙", homepage_sections: "◇", home_services: "▥", about_us: "◎", contact_us: "☎", destinations: "✦",
...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (session && active !== "dashboard" && active !== "media" && active !== "about_us" && active !== "contact_us" && active !== "home_services") loadTable(active);
  ...
  return <main className="adminShell">
    ...
    <section className="adminWorkspace">
      ...
      {active === "dashboard" ? <控制台 onNavigate={setActive} /> : active === "media" ? <MediaLibrary /> : active === "home_services" ? <HomeServicesManager /> : active === "homepage_sections" ? <HeroManager rows={rows} onRefresh={() => loadTable("homepage_sections")} /> : active === "destinations" ? <DestinationManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("destinations")} /> : active === "tours" ? <TourManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("tours")} /> : active === "site_settings" ? <SiteChromeManager /> : active === "about_us" ? <AboutManager /> : active === "contact_us" ? <ContactManager /> : active === "travel_guides" ? <GuideManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("travel_guides")} /> : active === "inquiries" ? <InquiryManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("inquiries")} /> : <Collection table={active} title={title} rows={rows} loading={loading} message={message} onRefresh={() => loadTable(active)} />}
    </section>
  </main>;
}

function HomeServicesManager(){const supabase=useMemo(()=>createClient(),[]);const base=[{title:"China Travel Concierge",text:"Personal support for planning your private China journey.",image:"",icon:"◎"},{title:"Fully Managed Trips",text:"Multi-city journeys managed from planning to daily coordination.",image:"",icon:"▦"},{title:"Itinerary Planning",text:"Smart routes, daily schedules and a comfortable travel pace.",image:"",icon:"⌖"},{title:"Driver & Guide",text:"Private drivers and professional English-speaking local guides.",image:"",icon:"◆"},{title:"Private China Tours",text:"Well-designed experiences for couples, families and small groups.",image:"",icon:"●"}];const [items,setItems]=useState(base);const [msg,setMsg]=useState("");useEffect(()=>{supabase.from("homepage_sections").select("content").eq("section_key","home:services").maybeSingle().then(({data})=>{if(data?.content)try{const x=typeof data.content==="string"?JSON.parse(data.content):data.content;if(Array.isArray(x?.items))setItems(x.items)}catch{}})},[supabase]);const update=(i:number,key:string,value:string)=>setItems(items.map((x,j)=>j===i?{...x,[key]:value}:x));async function upload(i:number,file?:File){if(!file)return;setMsg("图片上传中…");const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,"-");const path="homepage-services/"+Date.now()+"-"+safe;const {error}=await supabase.storage.from("website-media").upload(path,file);if(error){setMsg("上传失败："+error.message);return}const {data}=supabase.storage.from("website-media").getPublicUrl(path);update(i,"image",data.publicUrl);setMsg("图片已上传，请点击保存专业服务。")}async function save(){const {data}=await supabase.from("homepage_sections").select("id").eq("section_key","home:services").maybeSingle();const payload={section_key:"home:services",section_name:"首页专业服务",title:"Professional Services",content:JSON.stringify({items}),is_active:true};const {error}=data?.id?await supabase.from("homepage_sections").update(payload).eq("id",data.id):await supabase.from("homepage_sections").insert(payload);setMsg(error?"保存失败："+error.message:"专业服务已保存并同步到首页。")}return <section className="adminPanel"><header><div><h2>首页专业服务</h2><p>管理首页 Hero 下方五个服务卡片的图片、标题和说明。</p></div><a href="/" target="_blank" rel="noreferrer">查看前台 ↗</a></header><div className="serviceAdminGrid">{items.map((x:any,i:number)=><article className="destinationCard" key={i}><div className="serviceAdminPreview" style={x.image?{backgroundImage:"url('"+x.image+"')"}:undefined}><span>{x.icon||"✦"}</span></div><label><span>标题</span><input value={x.title||""} onChange={e=>update(i,"title",e.target.value)}/></label><label><span>简短说明</span><textarea rows={3} value={x.text||""} onChange={e=>update(i,"text",e.target.value)}/></label><label><span>图片</span><input type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={e=>upload(i,e.target.files?.[0])}/></label></article>)}</div>{msg&&<p className="tourSuccess">{msg}</p>}<button className="adminPrimary" onClick={save}>保存专业服务</button></section>}

function ContactManager(){const supabase=useMemo(()=>createClient(),[]);const base={introEyebrow:"LET'S TALK",introTitle:"Planning a trip to China?",introText:"Tell us what you have in mind. Our China-based travel team can help with private journeys, tailor-made itineraries and practical questions before you travel.",email:"hello@seekchinatravel.com",whatsapp:"",phone:"",address:"China",hours:"Monday – Sunday · 09:00 – 21:00 China Time",response:"We usually reply within 24 hours.",noteTitle:"Start with a simple idea.",noteText:"You do not need a finished itinerary. Tell us your preferred destinations, travel dates, group size and the experiences that matter to you. We will help shape the journey from there.",ctaTitle:"Ready to plan your China journey?",ctaText:"Share a few details with us and our team will help you take the next step."};const [d,setD]=useState(base);const [msg,setMsg]=useState("");useEffect(()=>{supabase.from("homepage_sections").select("content").eq("section_key","page:contact").maybeSingle().then(({data})=>{if(data?.content)try{setD({...base,...(typeof data.content==="string"?JSON.parse(data.content):data.content)})}catch{}})},[supabase]);async function save(){const {data}=await supabase.from("homepage_sections").select("id").eq("section_key","page:contact").maybeSingle();const payload={section_key:"page:contact",section_name:"Contact Us 页面",title:"Contact Us",content:JSON.stringify(d),is_active:true};const {error}=data?.id?await supabase.from("homepage_sections").update(payload).eq("id",data.id):await supabase.from("homepage_sections").insert(payload);setMsg(error?"保存失败："+error.message:"联系方式已保存并同步到前台。")}const field=(label:string,key:keyof typeof base,area=false)=><label><span>{label}</span>{area?<textarea rows={4} value={d[key]} onChange={e=>setD({...d,[key]:e.target.value})}/>:<input value={d[key]} onChange={e=>setD({...d,[key]:e.target.value})}/>}</label>;return <section className="adminPanel"><header><div><h2>联系方式</h2><p>管理 Contact Us 页面联系方式和说明文字。顶部 Hero 在「首页管理 → Contact Us 页面」修改。</p></div><a href="/contact" target="_blank" rel="noreferrer">查看前台 ↗</a></header><div className="destinationCard"><div className="destinationFormGrid">{field("小标题","introEyebrow")}{field("主标题","introTitle")}{field("介绍文字","introText",true)}{field("Email","email")}{field("WhatsApp（建议填写国际区号）","whatsapp")}{field("联系电话","phone")}{field("所在地区 / 地址","address")}{field("服务时间","hours")}{field("回复时间说明","response")}{field("说明板块标题","noteTitle")}{field("说明板块正文","noteText",true)}{field("底部 CTA 标题","ctaTitle")}{field("底部 CTA 正文","ctaText",true)}</div>{msg&&<p className="tourSuccess">{msg}</p>}<button className="adminPrimary" onClick={save}>保存联系方式</button></div></section>}

function AboutManager(){const supabase=useMemo(()=>createClient(),[]);const base={heroEyebrow:"ABOUT SEEK CHINA TRAVEL",heroTitle:"China, made personal.",heroText:"We are a China-based travel team creating private and tailor-made journeys for international travelers who want to experience the country with more ease, depth and confidence.",heroButton:"Plan Your China Trip →",heroLink:"/quote",mainImage:"",smallImage:"",storyEyebrow:"WHO WE ARE",storyTitle:"Local knowledge. Thoughtful journeys.",storyText:"",visualEyebrow:"BEYOND THE LANDMARKS",visualTitle:"See more of the China in between.",visualImage:"",detailContent:"",stats:[["15+","Locations"],["500+","Professional Guides"],["120,000+","Travelers Served Annually"],["140+","Selected Hotels & Stays"]],values:[["China-based expertise","Planning grounded in local destination knowledge, logistics and real travel experience."],["Designed around you","Private and flexible journeys shaped around your dates, pace, interests and travel style."],["Clear & dependable","Thoughtful arrangements, clear inclusions and support from planning through your journey."],["More than sightseeing","We help travelers connect with China's culture, landscapes, food and everyday life."]],approachEyebrow:"OUR APPROACH",approachTitle:"Discover China.\nYour Way.",approachText:"",ctaEyebrow:"START YOUR JOURNEY",ctaTitle:"Ready to see China differently?",ctaText:""};const [d,setD]=useState<any>(base);const [msg,setMsg]=useState("");useEffect(()=>{supabase.from("homepage_sections").select("content").eq("section_key","page:about").maybeSingle().then(({data})=>{if(data?.content)try{setD({...base,...(typeof data.content==="string"?JSON.parse(data.content):data.content)})}catch{}})},[supabase]);async function upload(key:string,file?:File){if(!file)return;const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,"-");const path="about/"+Date.now()+"-"+safe;const {error}=await supabase.storage.from("website-media").upload(path,file);if(error){setMsg(error.message);return}const {data}=supabase.storage.from("website-media").getPublicUrl(path);setD((x:any)=>({...x,[key]:data.publicUrl}));setMsg("图片已上传，请点击保存。")}async function save(){const {data}=await supabase.from("homepage_sections").select("id").eq("section_key","page:about").maybeSingle();const payload={section_key:"page:about",section_name:"About Us 页面",title:"About Us",content:JSON.stringify(d),is_active:true};const {error}=data?.id?await supabase.from("homepage_sections").update(payload).eq("id",data.id):await supabase.from("homepage_sections").insert(payload);setMsg(error?"保存失败："+error.message:"About Us 已保存并同步到前台。")}const field=(label:string,key:string,area=false)=><label><span>{label}</span>{area?<textarea rows={4} value={d[key]||""} onChange={e=>setD({...d,[key]:e.target.value})}/>:<input value={d[key]||""} onChange={e=>setD({...d,[key]:e.target.value})}/>}</label>;const image=(label:string,key:string)=><label><span>{label}</span>{d[key]&&<img src={d[key]} alt="" style={{width:"100%",height:120,objectFit:"cover",marginBottom:8,borderRadius:8}}/>}<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={e=>upload(key,e.target.files?.[0])}/></label>;return <section className="adminPanel"><header><div><h2>About Us 页面</h2><p>管理公司故事、图片和品牌理念。顶部 Hero 请在「首页管理 → About Us 页面」统一修改。</p></div><a href="/about" target="_blank" rel="noreferrer">查看前台 ↗</a></header><div className="destinationCard"><div className="destinationFormGrid">{image("公司故事主图","mainImage")}{image("公司故事叠加小图","smallImage")}{field("Who We Are 小标题","storyEyebrow")}{field("Who We Are 标题","storyTitle")}{field("公司简介正文","storyText",true)}{image("中间全宽背景图","visualImage")}{field("中间图片小标题","visualEyebrow")}{field("中间图片标题","visualTitle")}{d.values.map((v:any[],i:number)=><div key={i} style={{display:"grid",gap:8}}><b>理念 {i+1}</b><input value={v[0]} onChange={e=>{const x=d.values.map((z:any[])=>[...z]);x[i][0]=e.target.value;setD({...d,values:x})}}/><textarea rows={3} value={v[1]} onChange={e=>{const x=d.values.map((z:any[])=>[...z]);x[i][1]=e.target.value;setD({...d,values:x})}}/></div>)}<div style={{gridColumn:"1/-1"}}><b style={{display:"block",marginBottom:10}}>SCT 数据展示</b><p style={{margin:"0 0 12px",color:"#708494",fontSize:12}}>显示在 01–04 品牌理念下方。数字和说明都可以直接修改。</p><div className="chromeFooterGrid">{d.stats.map((v:any[],i:number)=><div key={i} style={{display:"grid",gap:8}}><input value={v[0]} placeholder="数字，例如 15+" onChange={e=>{const x=d.stats.map((z:any[])=>[...z]);x[i][0]=e.target.value;setD({...d,stats:x})}}/><input value={v[1]} placeholder="说明，例如 Locations" onChange={e=>{const x=d.stats.map((z:any[])=>[...z]);x[i][1]=e.target.value;setD({...d,stats:x})}}/></div>)}</div></div><div style={{gridColumn:"1/-1"}}><b style={{display:"block",marginBottom:10}}>详细介绍</b><p style={{margin:"0 0 12px",color:"#708494",fontSize:12}}>显示在 01–04 品牌理念下方，可使用标题、段落、列表、链接和图片。</p><RichTextEditor label="详细介绍" value={d.detailContent||""} onChange={(html)=>setD({...d,detailContent:html})}/></div>{field("Our Approach 小标题","approachEyebrow")}{field("Our Approach 标题","approachTitle")}{field("Our Approach 正文","approachText",true)}{field("底部 CTA 小标题","ctaEyebrow")}{field("底部 CTA 标题","ctaTitle")}{field("底部 CTA 正文","ctaText",true)}</div>{msg&&<p className="tourSuccess">{msg}</p>}<button className="adminPrimary" onClick={save}>保存 About Us</button></div></section>}

function SiteChromeManager(){const supabase=useMemo(()=>createClient(),[]);const base={header:"Home|/\nTours|/tours\nDestinations|/destinations\nTravel Guide|/travel-guide\nAbout Us|/about\nContact Us|/contact",footerExplore:"Tours|/tours\nDestinations|/destinations\nTravel Guide|/travel-guide\nAbout Us|/about",footerPlan:"Tailor-made Trips|/quote\nSmall Groups|/quote\nGet a Quote|/quote",footerConnect:"Email Us|mailto:hello@seekchinatravel.com\nContact Us|/quote",ctaLabel:"Get a Quote",ctaHref:"/quote",footerText:"Meaningful journeys, thoughtfully designed by people who call China home.",footerSlogan:"Discover China. Your Way.",logoUrl:"",whatsapp:"",floatingContact:true};const [d,setD]=useState(base);const [msg,setMsg]=useState("");const [saving,setSaving]=useState(false);const [uploading,setUploading]=useState(false);useEffect(()=>{supabase.from("homepage_sections").select("*").eq("section_key","site:chrome").maybeSingle().then(({data})=>{if(!data?.content)return;try{const x=typeof data.content==="string"?JSON.parse(data.content):data.content;const lines=(a:{label:string;href:string}[])=>(a||[]).map(v=>v.label+"|"+v.href).join("\n");setD({...base,header:lines(x.header),footerExplore:lines(x.footerExplore),footerPlan:lines(x.footerPlan),footerConnect:lines(x.footerConnect),ctaLabel:x.headerCta?.label||base.ctaLabel,ctaHref:x.headerCta?.href||base.ctaHref,footerText:x.footerText||base.footerText,footerSlogan:x.footerSlogan||base.footerSlogan,logoUrl:x.logoUrl||"",whatsapp:x.whatsapp||"",floatingContact:x.floatingContact!==false});}catch{}})},[supabase]);const parse=(v:string)=>v.split("\n").map(x=>x.trim()).filter(Boolean).map(x=>{const n=x.indexOf("|");return{label:(n<0?x:x.slice(0,n)).trim(),href:(n<0?"#":x.slice(n+1)).trim()}});async function uploadLogo(file?:File){if(!file)return;setUploading(true);setMsg("");const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,"-");const path="branding/logo-"+Date.now()+"-"+safe;const {error}=await supabase.storage.from("website-media").upload(path,file);if(error)setMsg("Logo 上传失败："+error.message);else{const {data}=supabase.storage.from("website-media").getPublicUrl(path);setD(v=>({...v,logoUrl:data.publicUrl}));setMsg("Logo 已上传，请点击保存。");}setUploading(false)}async function save(){setSaving(true);setMsg("");const content={header:parse(d.header),footerExplore:parse(d.footerExplore),footerPlan:parse(d.footerPlan),footerConnect:parse(d.footerConnect),headerCta:{label:d.ctaLabel,href:d.ctaHref},footerText:d.footerText,footerSlogan:d.footerSlogan,logoUrl:d.logoUrl,whatsapp:d.whatsapp,floatingContact:d.floatingContact};const {data}=await supabase.from("homepage_sections").select("id").eq("section_key","site:chrome").maybeSingle();const payload={section_key:"site:chrome",section_name:"顶部导航与页脚",title:"Site Navigation",content:JSON.stringify(content),is_active:true};const {error}=data?.id?await supabase.from("homepage_sections").update(payload).eq("id",data.id):await supabase.from("homepage_sections").insert(payload);setMsg(error?"保存失败："+error.message:"已保存，Logo、顶部导航和页脚已同步到前台。");setSaving(false)}const area=(label:string,key:"header"|"footerExplore"|"footerPlan"|"footerConnect")=><label className="chromeLinkField"><span>{label}</span><textarea rows={5} value={d[key]} onChange={e=>setD({...d,[key]:e.target.value})}/><small>每行一个：显示文字|链接地址</small></label>;return <section className="adminPanel siteChromeAdmin"><header><div><h2>顶部导航与页脚</h2><p>管理网站 Logo、顶部菜单、按钮及底部文字链接。</p></div></header><div className="destinationCard"><div className="siteChromeSection"><div className="destinationCardTitle"><b>品牌 Logo</b><span>上传后同时用于网站顶部和底部。</span></div>{d.logoUrl&&<div className="siteLogoPreview"><img src={d.logoUrl} alt="Current website logo"/></div>}<label className="chromeUpload"><span>上传 Logo</span><input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" disabled={uploading} onChange={e=>uploadLogo(e.target.files?.[0])}/><small>建议使用透明背景 PNG / WebP / SVG。</small></label></div><div className="siteChromeSection"><div className="destinationCardTitle"><b>顶部导航</b><span>一行一个菜单，文字和链接分别填写。</span></div>{area("顶部导航菜单","header")}<div className="chromeTwoCols"><label><span>顶部按钮文字</span><input value={d.ctaLabel} onChange={e=>setD({...d,ctaLabel:e.target.value})}/></label><label><span>顶部按钮链接</span><input value={d.ctaHref} onChange={e=>setD({...d,ctaHref:e.target.value})}/></label></div></div><div className="siteChromeSection"><div className="destinationCardTitle"><b>底部 Footer</b><span>分别管理三组链接及品牌文字。</span></div><div className="chromeFooterGrid">{area("Explore 链接","footerExplore")}{area("Plan 链接","footerPlan")}{area("Connect 链接","footerConnect")}</div><label className="chromeLinkField"><span>底部公司简介</span><textarea rows={3} value={d.footerText} onChange={e=>setD({...d,footerText:e.target.value})}/></label><label className="chromeLinkField"><span>底部品牌标语</span><input value={d.footerSlogan} onChange={e=>setD({...d,footerSlogan:e.target.value})}/></label><div className="chromeTwoCols"><label><span>漂浮联系框 WhatsApp</span><input value={d.whatsapp} placeholder="例如 +8613800000000" onChange={e=>setD({...d,whatsapp:e.target.value})}/></label><label><span>漂浮联系框</span><select value={d.floatingContact?"on":"off"} onChange={e=>setD({...d,floatingContact:e.target.value==="on"})}><option value="on">显示</option><option value="off">隐藏</option></select></label></div></div>{msg&&<p className="tourSuccess">{msg}</p>}<button className="adminPrimary" disabled={saving} onClick={save}>{saving?"正在保存…":"保存网站设置"}</button></div></section>}
