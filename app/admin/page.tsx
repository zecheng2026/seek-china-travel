...
function TourManager({rows,loading,message,onRefresh}:{rows:RecordRow[];loading:boolean;message:string;onRefresh:()=>void}){
 ...
 async function saveDay(day:RecordRow){if(day.id==null){setActionMessage("保存当天失败：缺少行程记录 ID，请刷新后重试。");return;}setActionMessage("正在保存 DAY "+String(day.day_number??"")+"…");const payload=Object.fromEntries(Object.entries(day).filter(([k])=>!["id","created_at","updated_at"].includes(k)).map(([k,v])=>[k,v===""?null:v]));const {data,error}=await supabase.from("tour_days").update(payload).eq("id",day.id).select().single();if(error){setActionMessage("保存当天失败："+error.message);return;}if(data){setDays(cur=>cur.map(d=>d.id===day.id?data as RecordRow:d));setActionMessage("DAY "+String((data as RecordRow).day_number??day.day_number??"")+" 已保存。");}else setActionMessage("保存当天失败：数据库没有返回更新后的记录。");}
