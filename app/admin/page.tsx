...
function TourManager({rows,loading,message,onRefresh}:{rows:RecordRow[];loading:boolean;message:string;onRefresh:()=>void}){
 ...
 function openEditor(row?:RecordRow){setActionMessage("");if(row){setEditing(row);setDraft({...row});loadDays(row.id);return;}const template=rows[0]?Object.fromEntries(Object.entries(rows[0]).filter(([k])=>!["id","created_at","updated_at"].includes(k)).map(([k,v])=>[k,typeof v==="boolean"?false:""])):{};setEditing({});setDraft(template);setDays([]);}
 ...
 async function deleteDay(day:RecordRow){if(day.id==null||!confirm("确定删除这一天的行程吗？"))return;const {error}=await supabase.from("tour_days").delete().eq("id",day.id);if(error)setActionMessage(error.message);else loadDays(editing?.id);}
 const fields=editing?Object.keys(draft).filter(k=>!["id","created_at","updated_at"].includes(k)):[];const imageFields=fields.filter(isImageField);const toggles=fields.filter(f=>typeof draft[f]==="boolean"||/^(is_|featured$|published$)/.test(f));const longFields=fields.filter(f=>!imageFields.includes(f)&&!toggles.includes(f)&&isLongField(f,draft[f]));const basics=fields.filter(f=>!imageFields.includes(f)&&!toggles.includes(f)&&!longFields.includes(f));
 if(editing)return <section className="destinationEditor tourEditor"><header className="destinationEditorHead"><div><button className="adminBack" onClick={()=>setEditing(null)}>← 返回线路列表</button><p>旅游线路编辑</p><h2>{editing.id==null?"新增旅游线路":String(draft.name??draft.title??"编辑线路")}</h2><span>先保存基本信息，再编辑 Day 1、Day 2 等每日行程。</span></div><div className="destinationEditorActions"><button onClick={()=>setEditing(null)}>返回</button><button className="adminPrimary" disabled={saving||fields.length===0} onClick={save}>{saving?"正在保存…":"保存线路"}</button></div></header>
 {actionMessage&&<div className={actionMessage.includes("已保存")?"tourSuccess":"adminEmpty error destinationError"}><p>{actionMessage}</p></div>}
 <div className="destinationCard"><div className="destinationCardTitle"><b>线路基本信息</b><span>名称、链接、目的地、天数、价格等游客决策信息。</span></div><div className="destinationFormGrid">{basics.map(field=><label key={field}><span>{tourLabel(field)}</span><input value={formatValue(draft[field])==="—"?"":formatValue(draft[field])} onChange={e=>setDraft({...draft,[field]:e.target.value})}/></label>)}</div></div>
 {imageFields.length>0&&<div className="destinationCard"><div className="destinationCardTitle"><b>线路图片</b><span>上传线路封面或 Banner 图片。</span></div><div className="destinationImages">{imageFields.map(field=>{const url=typeof draft[field]==="string"?String(draft[field]):"";return <div className="destinationImageBox" key={field}><span>{tourLabel(field)}</span>{url?<img src={url} alt=""/>:<div className="destinationImageEmpty">暂无图片</div>}<div><label className="destinationUpload">{uploading===field?"上传中…":"上传 / 更换图片"}<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" disabled={!!uploading} onChange={e=>uploadImage(field,e.target.files?.[0])}/></label>{url&&<button onClick={()=>setDraft({...draft,[field]:""})}>移除</button>}</div></div>})}</div></div>}
 {longFields.length>0&&<div className="destinationCard"><div className="destinationCardTitle"><b>线路介绍与费用说明</b><span>线路详细介绍支持富文本、链接及正文图片。</span></div><div className="destinationLongFields">{longFields.map(field=>field==="short_description"?<label key={field}><span>{tourLabel(field)}</span><textarea rows={4} value={formatValue(draft[field])==="—"?"":formatValue(draft[field])} onChange={e=>setDraft({...draft,[field]:e.target.value})}/></label>:<RichTextEditor key={field} label={tourLabel(field)} value={formatValue(draft[field])==="—"?"":formatValue(draft[field])} onChange={value=>setDraft(cur=>({...cur,[field]:value}))}/>)}</div></div>}
 <div className="destinationCard"><div className="destinationCardTitle tourDayTitle"><div><b>每日行程</b><span>管理 Day 1、Day 2、Day 3… 的具体安排。</span></div><button onClick={addDay}>＋ 添加一天</button></div>{editing.id==null?<div className="tourDayEmpty">请先点击“保存线路”，然后即可添加每日行程。</div>:daysLoading?<div className="tourDayEmpty">正在读取每日行程…</div>:days.length===0?<div className="tourDayEmpty">还没有每日行程，点击“添加一天”开始。</div>:<div className="tourDays">{days.map((day,index)=><div className="tourDayCard" key={String(day.id??index)}><div className="tourDayNumber">DAY <b>{String(day.day_number??index+1).padStart(2,"0")}</b></div><div className="tourDayFields">{Object.keys(day).filter(k=>!["id","tour_id","created_at","updated_at","day_number"].includes(k)).map(field=><label key={field}><span>{field==="title"?"当天标题":field==="description"||field==="content"?"行程内容":field==="meals"?"餐食":field==="hotel"?"住宿":field.replaceAll("_"," ")}</span>{/description|content|summary/i.test(field)?<textarea rows={4} value={formatValue(day[field])==="—"?"":formatValue(day[field])} onChange={e=>setDays(cur=>cur.map(d=>d.id===day.id?{...d,[field]:e.target.value}:d))}/>:<input value={formatValue(day[field])==="—"?"":formatValue(day[field])} onChange={e=>setDays(cur=>cur.map(d=>d.id===day.id?{...d,[field]:e.target.value}:d))}/>}</label>)}</div><div className="tourDayActions"><button onClick={()=>saveDay(day)}>保存当天</button><button className="danger" onClick={()=>deleteDay(day)}>删除</button></div></div>)}</div>}</div>
 {toggles.length>0&&<div className="destinationCard"><div className="destinationCardTitle"><b>前台设置</b><span>控制线路发布、推荐等展示状态。</span></div><div className="destinationToggles">{toggles.map(field=><label key={field}><div><b>{tourLabel(field)}</b><small>控制该线路在网站上的展示。</small></div><input type="checkbox" checked={Boolean(draft[field])} onChange={e=>setDraft({...draft,[field]:e.target.checked})}/></label>)}</div></div>}
 <div className="destinationBottomActions"><button onClick={()=>setEditing(null)}>返回列表</button><button className="adminPrimary" disabled={saving} onClick={save}>{saving?"正在保存…":"保存线路"}</button></div></section>;
