...
export default function AdminPage() {
  ...
  return <main className="adminShell">
    ...
    <section className="adminWorkspace">
      ...
      {active === "dashboard" ? <控制台 onNavigate={setActive} /> : active === "media" ? <MediaLibrary /> : active === "destinations" ? <DestinationManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("destinations")} /> : active === "tours" ? <TourManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("tours")} /> : <Collection table={active} title={title} rows={rows} loading={loading} message={message} onRefresh={() => loadTable(active)} />}
    </section>
  </main>;
}
...
const tourLabels: Record<string,string> = {name:"线路英文名称",title:"线路英文名称",slug:"URL 标识",subtitle:"线路副标题",destination:"目的地",destination_id:"目的地 ID",duration:"行程天数",duration_days:"行程天数",days:"行程天数",price:"价格",price_from:"起价",original_price:"原价",currency:"币种",short_description:"线路简介",description:"详细介绍",content:"详细内容",highlights:"行程亮点",included:"费用包含",includes:"费用包含",excluded:"费用不含",excludes:"费用不含",hotel:"住宿说明",hotels:"住宿说明",meals:"餐食说明",hero_image_url:"线路封面",image_url:"线路封面",cover_image_url:"线路封面",gallery_images:"线路图片集",important_notes:"重要提示",suitable_for:"适合人群",hotel_description:"住宿说明",meals_description:"餐食说明",transportation_description:"交通说明",is_featured:"首页推荐",featured:"首页推荐",is_published:"发布到网站",published:"发布到网站",sort_order:"显示顺序",display_order:"显示顺序"};
...
function TourManager({rows,loading,message,onRefresh}:{rows:RecordRow[];loading:boolean;message:string;onRefresh:()=>void}){
 const supabase=useMemo(()=>createClient(),[]);const [editing,setEditing]=useState<RecordRow|null>(null);const [draft,setDraft]=useState<RecordRow>({});const [days,setDays]=useState<RecordRow[]>([]);const [daysLoading,setDaysLoading]=useState(false);const [saving,setSaving]=useState(false);const [uploading,setUploading]=useState("");const [actionMessage,setActionMessage]=useState("");
 ...
 function openEditor(row?:RecordRow){setActionMessage("");if(row){setEditing(row);setDraft({...row});loadDays(row.id);return;}const template:RecordRow={name:"",slug:"",subtitle:"",destination:"",duration_days:"",price_from:"",original_price:"",currency:"CNY",short_description:"",description:"",highlights:"",hero_image_url:"",gallery_images:[],included:"",excluded:"",important_notes:"",suitable_for:"",hotel_description:"",meals_description:"",transportation_description:"",is_featured:false,is_published:false,sort_order:0};setEditing({});setDraft(template);setDays([]);}
 async function uploadImage(field:string,file?:File){if(!file)return;setUploading(field);setActionMessage("");const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,"-");const path="tours/"+Date.now()+"-"+safe;const {error}=await supabase.storage.from("website-media").upload(path,file,{upsert:false});if(error)setActionMessage("图片上传失败："+error.message);else{const {data}=supabase.storage.from("website-media").getPublicUrl(path);setDraft(cur=>({...cur,[field]:data.publicUrl}));}setUploading("");} async function uploadGallery(files?:FileList|null){if(!files?.length)return;setUploading("gallery_images");setActionMessage("");const current=Array.isArray(draft.gallery_images)?draft.gallery_images.filter(x=>typeof x==="string") as string[]:[];const urls:string[]=[];for(const file of Array.from(files)){const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,"-");const path="tours/gallery/"+Date.now()+"-"+Math.random().toString(36).slice(2,7)+"-"+safe;const {error}=await supabase.storage.from("website-media").upload(path,file,{upsert:false});if(error){setActionMessage("相册图片上传失败："+error.message);continue;}const {data}=supabase.storage.from("website-media").getPublicUrl(path);urls.push(data.publicUrl);}setDraft(cur=>({...cur,gallery_images:[...current,...urls]}));setUploading("");} function removeGalleryImage(url:string){const current=Array.isArray(draft.gallery_images)?draft.gallery_images.filter(x=>typeof x==="string"&&x!==url):[];setDraft(cur=>({...cur,gallery_images:current}));}
 async function save(){setSaving(true);setActionMessage("");const payload=Object.fromEntries(Object.entries(draft).filter(([k])=>!["id","created_at","updated_at"].includes(k)).map(([k,v])=>[k,v===""?null:v]));const id=editing?.id;const query=id==null?supabase.from("tours").insert(payload).select().single():supabase.from("tours").update(payload).eq("id",id).select().single();const {data,error}=await query;if(error)setActionMessage(error.message);else if(data){setEditing(data as RecordRow);setDraft(data as RecordRow);await loadDays((data as RecordRow).id);onRefresh();setActionMessage("线路已保存。");}setSaving(false);}
 ...
 async function addDay(){if(editing?.id==null){setActionMessage("请先保存线路基本信息，再添加每日行程。");return;}const sample=days[0];const payload:RecordRow={tour_id:editing.id,day_number:days.length+1};if(sample)Object.keys(sample).filter(k=>!["id","tour_id","created_at","updated_at","day_number"].includes(k)).forEach(k=>payload[k]=null);const {error}=await supabase.from("tour_days").insert(payload);if(error)setActionMessage(error.message);else loadDays(editing.id);}
 async function saveDay(day:RecordRow){if(day.id==null)return;const payload=Object.fromEntries(Object.entries(day).filter(([k])=>!["id","created_at","updated_at"].includes(k)).map(([k,v])=>[k,v===""?null:v]));const {error}=await supabase.from("tour_days").update(payload).eq("id",day.id);if(error)setActionMessage(error.message);else setActionMessage("每日行程已保存。");}
 ...
 const fields=editing?Object.keys(draft).filter(k=>!["id","created_at","updated_at"].includes(k)):[];const imageFields=fields.filter(isImageField);const toggles=fields.filter(f=>typeof draft[f]==="boolean"||/^(is_|featured$|published$)/.test(f));const basicKeys=["name","title","slug","subtitle","destination","destination_id","duration","duration_days","days","suitable_for","sort_order"];const priceKeys=["price","price_from","original_price","currency"];const introKeys=["short_description","description","content","highlights"];const feeKeys=["included","includes","excluded","excludes"];const serviceKeys=["hotel","hotels","meals","hotel_description","meals_description","transportation_description"];const noteKeys=["important_notes"];const basics=fields.filter(f=>basicKeys.includes(f));const prices=fields.filter(f=>priceKeys.includes(f));const intros=fields.filter(f=>introKeys.includes(f));const fees=fields.filter(f=>feeKeys.includes(f));const services=fields.filter(f=>serviceKeys.includes(f));const notes=fields.filter(f=>noteKeys.includes(f));const handled=new Set([...imageFields,...toggles,...basics,...prices,...intros,...fees,...services,...notes]);const extras=fields.filter(f=>!handled.has(f));
 ...
 <div className="tourSectionNav"><span>产品录入顺序</span><b>01 基本信息</b><b>02 价格</b><b>03 图片</b><b>04 介绍亮点</b><b>05 每日行程</b><b>06 费用</b><b>07 服务</b><b>08 提示</b><b>09 发布</b></div>
 ...
 {imageFields.length>0&&<div className="destinationCard"><div className="destinationCardTitle"><b>03 · 封面与相册</b><span>上传线路主视觉；图片集字段保留用于后续多图相册。</span></div><div className="destinationImages">{imageFields.map(field=>{if(field==="gallery_images"){const gallery=Array.isArray(draft.gallery_images)?draft.gallery_images.filter(x=>typeof x==="string") as string[]:[];return <div className="tourGalleryManager" key={field}><div className="tourGalleryHead"><span>{tourLabel(field)}</span><label className="destinationUpload">{uploading==="gallery_images"?"上传中…":"＋ 批量上传相册"}<input type="file" multiple accept="image/jpeg,image/png,image/webp,image/avif" disabled={!!uploading} onChange={e=>uploadGallery(e.target.files)}/></label></div>{gallery.length?<div className="tourGalleryGrid">{gallery.map((url,index)=><div key={url+index}><img src={url} alt=""/><button type="button" onClick={()=>removeGalleryImage(url)}>移除</button></div>)}</div>:<div className="destinationImageEmpty">暂无相册图片，可一次选择多张上传</div>}<small>相册顺序按上传顺序展示；保存线路后同步到英文详情页。</small></div>}const url=typeof draft[field]==="string"?String(draft[field]):"";return <div className="destinationImageBox" key={field}><span>{tourLabel(field)}</span>{url?<img src={url} alt=""/>:<div className="destinationImageEmpty">暂无图片</div>}<div><label className="destinationUpload">{uploading===field?"上传中…":"上传 / 更换图片"}<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" disabled={!!uploading} onChange={e=>uploadImage(field,e.target.files?.[0])}/></label>{url&&<button onClick={()=>setDraft({...draft,[field]:""})}>移除</button>}</div></div>})}</div></div>}
 {intros.length>0&&<div className="destinationCard"><div className="destinationCardTitle"><b>04 · 线路简介与亮点</b><span>先写简短卖点，再完善详细介绍和行程亮点。</span></div><div className="destinationLongFields">{intros.map(field=>field==="short_description"||field==="highlights"?<label key={field}><span>{tourLabel(field)}</span><textarea rows={field==="highlights"?6:4} value={formatValue(draft[field])==="—"?"":formatValue(draft[field])} onChange={e=>setDraft({...draft,[field]:e.target.value})}/></label>:<RichTextEditor key={field} label={tourLabel(field)} value={formatValue(draft[field])==="—"?"":formatValue(draft[field])} onChange={value=>setDraft(cur=>({...cur,[field]:value}))}/>)}</div></div>}
 <div className="destinationCard"><div className="destinationCardTitle tourDayTitle"><div><b>05 · 每日行程</b><span>管理 Day 1、Day 2、Day 3…；当天图片可直接上传。</span></div><button onClick={addDay}>＋ 添加一天</button></div>{editing.id==null?<div className="tourDayEmpty">请先保存线路基本信息，然后即可添加每日行程。</div>:daysLoading?<div className="tourDayEmpty">正在读取每日行程…</div>:days.length===0?<div className="tourDayEmpty">还没有每日行程，点击“添加一天”开始。</div>:<div className="tourDays">{days.map((day,index)=><div className="tourDayCard" key={String(day.id??index)}><div className="tourDayNumber">DAY <b>{String(day.day_number??index+1).padStart(2,"0")}</b></div><div className="tourDayFields">{Object.keys(day).filter(k=>!["id","tour_id","created_at","updated_at","day_number","image_url"].includes(k)).map(field=><label key={field}><span>{field==="title"?"当天标题":field==="description"||field==="content"?"行程内容":field==="attractions"?"主要景点":field==="meals"?"餐食":field==="hotel"?"住宿":field==="transportation"?"交通":field.replaceAll("_"," ")}</span>{/description|content|summary/i.test(field)?<textarea rows={4} value={formatValue(day[field])==="—"?"":formatValue(day[field])} onChange={e=>setDays(cur=>cur.map(d=>d.id===day.id?{...d,[field]:e.target.value}:d))}/>:<input value={formatValue(day[field])==="—"?"":formatValue(day[field])} onChange={e=>setDays(cur=>cur.map(d=>d.id===day.id?{...d,[field]:e.target.value}:d))}/>}</label>)}<div className="tourDayImage"><span>当天图片</span>{day.image_url?<img src={String(day.image_url)} alt=""/>:<div>暂无图片</div>}<label>{uploading==="day-"+String(day.id)?"上传中…":"上传 / 更换当天图片"}<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" disabled={!!uploading} onChange={e=>uploadDayImage(day,e.target.files?.[0])}/></label></div></div><div className="tourDayActions"><button onClick={()=>saveDay(day)}>保存当天</button><button className="danger" onClick={()=>deleteDay(day)}>删除</button></div></div>)}</div>}</div>
 {fees.length>0&&<div className="destinationCard"><div className="destinationCardTitle"><b>06 · 费用说明</b><span>分别说明报价包含和不包含的项目。</span></div><div className="destinationLongFields">{fees.map(field=><RichTextEditor key={field} label={tourLabel(field)} value={formatValue(draft[field])==="—"?"":formatValue(draft[field])} onChange={value=>setDraft(cur=>({...cur,[field]:value}))}/>)}</div></div>}
 {services.length>0&&<div className="destinationCard"><div className="destinationCardTitle"><b>07 · 住宿 · 餐食 · 交通</b><span>集中填写服务标准，避免与基本信息混在一起。</span></div><div className="tourServiceGrid">{services.map(field=><label key={field}><span>{tourLabel(field)}</span><textarea rows={5} value={formatValue(draft[field])==="—"?"":formatValue(draft[field])} onChange={e=>setDraft({...draft,[field]:e.target.value})}/></label>)}</div></div>}
 {notes.length>0&&<div className="destinationCard"><div className="destinationCardTitle"><b>08 · 重要提示</b><span>填写预订、年龄、证件、天气或其他需要游客提前知道的事项。</span></div>{notes.map(field=><RichTextEditor key={field} label={tourLabel(field)} value={formatValue(draft[field])==="—"?"":formatValue(draft[field])} onChange={value=>setDraft(cur=>({...cur,[field]:value}))}/>)}</div>}
 {extras.length>0&&<div className="destinationCard"><div className="destinationCardTitle"><b>其他信息</b></div><div className="destinationFormGrid">{extras.map(field=><label key={field}><span>{tourLabel(field)}</span><input value={formatValue(draft[field])==="—"?"":formatValue(draft[field])} onChange={e=>setDraft({...draft,[field]:e.target.value})}/></label>)}</div></div>}
 {toggles.length>0&&<div className="destinationCard"><div className="destinationCardTitle"><b>09 · 发布设置</b><span>确认内容完成后再发布到网站或设为首页推荐。</span></div><div className="destinationToggles">{toggles.map(field=><label key={field}><div><b>{tourLabel(field)}</b><small>{/featured/.test(field)?"开启后可用于首页推荐区域。":"开启后允许前台展示该线路。"}</small></div><input type="checkbox" checked={Boolean(draft[field])} onChange={e=>setDraft({...draft,[field]:e.target.checked})}/></label>)}</div></div>}
