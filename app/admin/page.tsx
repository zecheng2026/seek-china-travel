...
function TourManager({rows,loading,message,onRefresh}:{rows:RecordRow[];loading:boolean;message:string;onRefresh:()=>void}){
 ...
 async function moveDay(day:RecordRow,direction:number){const current=Number(day.day_number??0);const target=days.find(d=>Number(d.day_number??0)===current+direction);if(!target||day.id==null||target.id==null)return;setActionMessage("正在调整每日行程顺序…");const {error:firstError}=await supabase.from("tour_days").update({day_number:current+direction}).eq("id",day.id);if(firstError){setActionMessage("调整顺序失败："+firstError.message);return;}const {error:secondError}=await supabase.from("tour_days").update({day_number:current}).eq("id",target.id);if(secondError){await supabase.from("tour_days").update({day_number:current}).eq("id",day.id);setActionMessage("调整顺序失败："+secondError.message);return;}await loadDays(editing?.id);setActionMessage("每日行程顺序已更新。");}
