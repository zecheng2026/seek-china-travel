...
const heroPages=[["home","首页"],["tours","Tours 页面"],["destinations","Destinations 页面"],["quote","Plan Your Trip 页面"]] as const;
...
export default function AdminPage() {
  ...
  return <main className="adminShell">
    ...
    <section className="adminWorkspace">
      ...
      {active === "dashboard" ? <控制台 onNavigate={setActive} /> : active === "media" ? <MediaLibrary /> : active === "homepage_sections" ? <HeroManager rows={rows} onRefresh={() => loadTable("homepage_sections")} /> : active === "destinations" ? <DestinationManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("destinations")} /> : active === "tours" ? <TourManager rows={rows} loading={loading} message={message} onRefresh={() => loadTable("tours")} /> : <Collection table={active} title={title} rows={rows} loading={loading} message={message} onRefresh={() => loadTable(active)} />}
    </section>
  </main>;
}
...
function TourManager({rows,loading,message,onRefresh}:{rows:RecordRow[];loading:boolean;message:string;onRefresh:()=>void}){
 ...
 function openEditor(row?:RecordRow){setActionMessage("");if(row){setEditing(row);setDraft({...row});loadDays(row.id);return;}const template:RecordRow={name:"",slug:"",subtitle:"",destination:"",duration_days:"",price_from:"",original_price:"",currency:"CNY",short_description:"",description:"",highlights:"",hero_image_url:"",included:"",excluded:"",important_notes:"",suitable_for:"",hotel_description:"",meals_description:"",transportation_description:"",is_featured:false,is_published:false,sort_order:0};setEditing({});setDraft(template);setDays([]);}
